...
memory manually.
See [attributes](./attributes/overview.md) for more information.
...
fn test() []int {
	...
	numbers := [1, 2, 3] // array allocated on heap, will be
                         // freed as the function exits
	println(number)
	println(user)
	println(numbers)
...
`c` is returned. For this reason `b` and `c` will be heaped allocated.
...
A solution to this dilemma is the `[heap]` [attribute](./attributes/overview.md) at the declaration
of `struct MyStruct`. It instructs the compiler to *always* allocate `MyStruct`-objects
