...
## Upgrading V to the latest version
...
You can check the current type of sum type using `is` and its negated form `!is`.
...
A sum type instance can hold a value of several types. Use the `type`
...
Channels have the type `chan objtype`. An optional buffer length can specify as the `cap` field
in the declaration:
